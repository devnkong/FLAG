...
def train_flag(data, dataset, model, optimizer, criterion, device, args):
    ...
    for idx in idx_clusters:
        ...
        forward = lambda perturb : model(x, sg_nodes_idx, sg_edges_, sg_edges_attr, perturb)[training_idx].to(torch.float32)
        model_forward = (model, forward)
        target = train_y[inter_idx].to(device).to(torch.float32)

        loss, out = flag(model_forward, (x.shape[0], args.hidden_channels), target, args, optimizer, device, criterion)
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        epoch_loss = train_flag(data, dataset, model, optimizer, criterion, device, args)
