# Hokulea Instructions

`--device ` and `—-start-seed` need to be set manually, each command will start 1 run, the final result requires mean and std of 10 runs.

`—-start-seed` sets the random seed of the run. We just need 10 runs with `—-start-seed` as 0-9. 

According to my prior experiments, each run will roughly take one day, so if workable we can run several in the meantime.

If resources or time are limited, I guess don't need to finish all the 10 runs for those `medium` and `standard` baselines. 3 or 4 runs should be fine.

Error log will be directed to local `error.log`.

Running evaluation statistics will be saved under `./logs`, could daily commit and push to the github repo to sync.

## ogbg-molpcba

High priority 

    python ./Hokulea/mol/main_pyg.py  —-gnn gin-virtual —-runs 1  —-same-epoch —-device () —-start-seed () > error.log 2>1&
    
Medium priority

    python ./Hokulea/mol/main_pyg.py  —-gnn gin —-runs 1  —-same-epoch —-device () —-start-seed () > error.log 2>1&
    
Standard priority    

    python ./Hokulea/mol/main_pyg.py  —-gnn gcn —-runs 1  —-same-epoch —-device () —-start-seed () > error.log 2>1&
    
## ogbg-ppa

High priority 

    python ./Hokulea/ppa/main_pyg.py  --gnn gin-virtual --runs 1 --same-epoch --device () --start-seed () > error.log 2>1&
    
Medium priority 

    python ./Hokulea/ppa/main_pyg.py  --gnn gin --runs 1 --same-epoch --device () --start-seed () > error.log 2>1&
    
Standard priority   

    python ./Hokulea/ppa/main_pyg.py  --gnn gcn --runs 1 --same-epoch --device () --start-seed () > error.log 2>1&
